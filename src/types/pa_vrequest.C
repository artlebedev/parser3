...
volatile const char * IDENT_PA_VREQUEST_C="$Id: pa_vrequest.C,v 1.53 2012/06/15 06:13:11 misha Exp $" IDENT_PA_VREQUEST_H;
...
Value* VRequest::get_element(const String& aname) {
	...
	if(aname==POST_BODY_NAME){
		...
		result.set(true/*tainted*/, false/*binary*/, (finfo.post_data)?finfo.post_data:""/*to distinguish from stat-ed file*/, finfo.post_size);
		return &result;
	}
